...
class Critic(nn.Module):
    def __init__(self, obs_dim, goal_dim, action_dim, hidden_dim):
        ...
        #need to add another for policy training? 
...
class WGCSLAgent:
    ...
    def update_actor(self, obs, goal, action, step, weights=None):
        if weights is None:
...
#         actor_loss = -Q.mean()
        ...
        #gcsl_weight_tf =???
        weighted_sl_loss = F.mse_loss(action, self.act(obs, goal, step, eval_mode=False))
        policy_sl_loss = gcsl_weight_tf * weighted_sl_loss.mean()
...
#         self.policy_sl_loss = tf.reduce_mean(self.gcsl_weight_tf * self.weighted_sl_loss)  #  + 0.01 * self.temp_action_loss
        ...
        policy_sl_loss.backward()
        self.actor_opt.step()
        
        #optimize policy_sl_loss
