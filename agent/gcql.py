...
class Actor(nn.Module):
    ...
    def forward(self, obs, goal):
        mu, log_std = self.policy(torch.concat([obs,goal],-1)).chunk(2, dim=-1)
...
class GCQLAgent:
    ...
    def _repeated_critic_apply(self, obs, goal, actions):
        """
        ...
        repeated_goal = goal.unsqueeze(0).repeat((n_samples, 1, 1)).float()
        repeated_goal = repeated_goal.reshape((n_samples * batch_size, -1))

        Q1, Q2 = self.critic(repeated_obs, repeated_goal, reshaped_actions)
        ...
        with torch.no_grad():
            random_actions = torch.FloatTensor(self.n_samples, Q1.shape[0],
                                               action.shape[-1]).uniform_(
                                                   -1, 1).to(self.device)
            sampled_actions = self.actor(obs, goal).sample(
                sample_shape=(self.n_samples,))
            next_sampled_actions = self.actor(next_obs, goal).sample(
                sample_shape=(self.n_samples,))

        rand_Q1, rand_Q2 = self._repeated_critic_apply(obs, goal, random_actions)
        sampled_Q1, sampled_Q2 = self._repeated_critic_apply(
            obs, goal, sampled_actions)
        next_sampled_Q1, next_sampled_Q2 = self._repeated_critic_apply(
            obs, goal, next_sampled_actions)
    ...
    def update_actor(self, obs, goal, action, step):
        ...
        # optimize actor
        Q1, Q2 = self.critic(obs, goal, sampled_action)
        Q = torch.min(Q1, Q2)
