...
class Actor(nn.Module):
    def __init__(self, obs_dim, goal_dim, action_dim, hidden_dim, num_horizon=20, horizon_embed_dim=4):
        super().__init__()
        self.horizon_embedding = nn.Embedding(num_horizon, horizon_embed_dim)

        self.policy = nn.Sequential(nn.Linear(obs_dim+goal_dim+0*horizon_embed_dim, hidden_dim),
