...
##how do we get goal dim & add goal
...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim, goal_dim, hidden_dim):
        ...
        #how to change forward when we add goal dim 
...
class Critic(nn.Module):
    ...
    def forward(self, obs, action, goal):
        obs_action_goal = torch.cat([obs, action, goal], dim=-1)
        q1 = self.q1_net(obs_action_goal)
        q2 = self.q2_net(obs_action_goal)
...
class TD3Agent:
    def __init__(self,
        ...
        # models
        self.actor = Actor(obs_shape[0], action_shape[0], goal_shape[0],
                           hidden_dim).to(device)

        self.critic = Critic(obs_shape[0], action_shape[0], goal_shape[0],
                             hidden_dim).to(device)
        self.critic_target = Critic(obs_shape[0], action_shape[0], goal_shape[0],
                                    hidden_dim).to(device)
    ...
    def act(self, obs, step, goal, eval_mode):
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, terminal, goal, step):
        metrics = dict()
    ...
    def update(self, replay_iter, step):
        ...
        obs, action, reward, discount, terminal, goal, next_obs = utils.to_torch(
            batch, self.device)
        ...
        # update critic
        metrics.update(
            self.update_critic(obs, action, reward, discount, terminal, goal, next_obs, step))
