...
def load_single_env(
  ...
  for name, space in env.act_space.items():
    if name == 'reset':
      continue
    if space.discrete:
      env = core.wrappers.OneHotAction(env, name)
    elif discretize:
      env = core.wrappers.DiscretizeAction(env, name, discretize)
    else:
      env = core.wrappers.NormalizeAction(env, name)
  if length:
    env = core.wrappers.TimeLimit(env, length)
  env = core.wrappers.ExpandScalars(env)
  if checks:
    env = core.wrappers.CheckSpaces(env)
