...
python pph_general.py agent=proto_encoder1 task_no_goal=$task_no_goal task=$task domain=point_mass_maze obs_type=pixels goal=True replay_buffer_gc=500000 num_train_frames=4000010 num_seed_frames=4000 replay_buffer_num_workers=4 num_protos=16 pred_dim=16 feature_dim=16 hidden_dim=256 batch_size_gc=1024 use_wandb=True eval_every_frames=20000 episode_length=500 lr=.0001 frame_stack=1 proto_goal_med=False proto_goal_intr=True proto_goal_random=False og=False combine_storage_gc=True switch_gc=100000 stddev_schedule=.2 stddev_clip=.2 stddev_schedule2=.2 stddev_clip2=.2 hybrid_gc=True hybrid_pct=.7 tmux_session=batch seed=$seed test1=True nstep1=3 nstep2=3 update_proto_opt=False update_proto_while_gc=True update_gc_while_proto=True domain=point_mass_maze"
EOT
#echo $msg
