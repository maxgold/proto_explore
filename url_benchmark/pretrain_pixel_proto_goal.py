...
def make_agent(obs_type, obs_spec, action_spec, goal_shape, num_expl_steps, goal, cfg, 
               ...
               stddev_schedule=.2, stddev_schedule2=.2,stddev_clip=.3,stddev_clip2=.3,proto_const_init=True):
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            if self._global_step%100000==0 and self._global_step>=500000:
                ...
                path = os.path.join(self.work_dir, 'actor2_{}_{}.pth'.format(str(self.cfg.agent.name),self._global_step))
                torch.save(self.agent.actor2, path)
                path = os.path.join(self.work_dir, 'critic2_{}_{}.pth'.format(str(self.cfg.agent.name),self._global_step))
                torch.save(self.agent.critic2, path)
                path = os.path.join(self.work_dir, 'protos_{}_{}.pth'.format(str(self.cfg.agent.name),self._global_step))
                torch.save(self.agent.protos, path)
                path = os.path.join(self.work_dir, 'predictor_{}_{}.pth'.format(str(self.cfg.agent.name),self._global_step))
                torch.save(self.agent.predictor, path)
                path = os.path.join(self.work_dir, 'projector_{}_{}.pth'.format(str(self.cfg.agent.name),self._global_step))
                torch.save(self.agent.projector, path)
