...
class Workspace:
    def __init__(self, cfg):
        ...
        self.first_goal = np.random.sample((2,)) * .5 - .25   
        ...
        self.replay_loader2  = make_replay_loader(self.replay_storage2,
                                                ...
                                                cfg.hybrid, cfg.obs_type,
                                                cfg.hybrid_pct,actor1=False)
    ...
    def eval_goal_proto_visual(self, proto, model):
        ...
        for ix, x in enumerate(goal_array):
            step, episode, total_reward = 0, 0, 0
            self.eval_env = dmc.make(self.cfg.task, seed=None, goal=np.array([x[0], x[1]]))
            eval_until_episode = utils.Until(self.cfg.num_eval_episodes)
            meta = self.agent.init_meta()
            while eval_until_episode(episode):
                time_step = self.eval_env.reset()
                ...
                if self.cfg.eval:
                    ...
                    save(str(self.work_dir)+'/eval_{}.csv'.format(self._global_step), [[x, total_reward, time_step.observation[:2], step]])
    ...
    def train(self):
        ...
        meta = self.agent.init_meta()
        self.replay_storage1.add_goal(time_step1, meta, self.first_goal)
        ...
        while train_until_step(self._global_step):
            ...
            if episode_step%1000==0 and self.global_step!=0:
                if seed_until_step(self._global_step):
                    ...
                    goal = self.sample_goal_uniform(time_step1.observation)
                self.train_env1 = dmc.make(self.cfg.task, seed=None, goal=goal)
...
def main(cfg):
    from mypretrain_concurrent import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
