...
class ReplayBuffer(IterableDataset):
    ...
    def _sample_goal_hybrid(self):
        ...
        if key > self.hybrid_pct:
            ...
            protos = self.agent.proto_goal.detach().clone().cpu().numpy()
            z_to_proto = np.linalg.norm(obs[None, :] - protos[:, :], axis=1, ord=2)
            z_to_proto_ = np.argsort(z_to_proto)
            protos[z_to_proto_[0]] = z
            z_to_proto = np.sort(z_to_proto)
            #print('state_no_reward', episode['state'][idx-1])
            #print('goal_no_reward', episode['state'][idx_goal])

            for i in range(1):
                obs_to_proto = np.linalg.norm(obs[None, :] - protos[:, :], axis=1, ord=2)
                obs_to_proto_sort = np.sort(obs_to_proto)
                dist = -(obs_to_proto[z_to_proto_[0]] - obs_to_proto_sort[0])/(obs_to_proto_sort[-1] - obs_to_proto_sort[0])
...
def make_replay_loader(
    storage,  storage2, max_size, batch_size, num_workers, save_snapshot, nstep, discount, goal, hybrid=False, obs_type='state', hybrid_pct=0, actor1=False, replay_dir2=False,model_step=False,goal_proto=False, agent=None, neg_reward=False,return_iterable=False):
    max_size_per_worker = max_size // max(1, num_workers)
