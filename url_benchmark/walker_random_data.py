...
def get_closest_physics(state, state_physics):
    states, physics = state_physics
    dists = ssd.cdist(state[None], states)
    assgn = dists.argmin(1)
    return physics[assgn]
...
def test_agent(
    ...
    rewards1 = []
    rewards2 = []
    start_physics = get_closest_physics(start, state_physics)[0]
    with env.physics.reset_context():
        env.physics.set_state(start_physics)
...
class ReplayBuffer2(IterableDataset):
    def __init__(self, episodes, max_horizon):
        self.episodes = episodes
        self.max_horizon = max_horizon

    def sample(self):
        ep_id = int(np.random.randint(0, len(self.episodes), 1))
        states, actions, _, _ = self.episodes[ep_id]
        idx = np.random.randint(0, len(states) - self.max_horizon)
        horizon = np.random.randint(1, self.max_horizon + 1)
        obs = states[idx]
        goal = states[idx + horizon]  # [:2]
        action = actions[idx]
...
def make_replay_loader(
    ...
    iterable = ReplayBuffer2(episodes, max_horizon)
...
task = "walker_walk"
expert_agent = torch.load("./exp_local/2022.10.31/173005_ddpgwalker/model.ckpt")
obs_type = "states"
frame_stack = 1
action_repeat = 1
seed = 1
...
expert_agent.actor(obs, 0.1).loc
...
for ep in tqdm.tqdm(range(num_episodes)):
    ...
    if obs_type == "states":
        ...
        states.append(get_emb(proto_agent, obs))
        obs.cpu()
    ...
    while not time_step.last():
        ...
        if strategy == "random":
            if step % 2 == 0:
                ...
                action = expert_agent.actor(torch.tensor(obs).cuda(), 1).loc.detach().cpu().numpy()
        else:
            action = expert_agent.actor(torch.tensor(obs).cuda(), 1).loc.detach().cpu().numpy()
        ...
        if time_step.last():
            break
    episodes.append((states, actions, physics, np.array(r)[None]))

import pickle

#with open("tmp_walker.pkl", "rb") as f:
#    episodes = pickle.load(f)

if False:
    with open("tmp_walker.pkl", "wb") as f:
        pickle.dump(episodes, f)

max_horizon = 20

loader = iter(make_replay_loader(episodes, 512, 0, max_horizon=max_horizon))

from agent.gcsl import GCSLAgent2

#goal_dim = (16,)
obs_dim = env.observation_spec().shape
goal_dim = env.observation_spec().shape
lr = 1e-3
hidden_dim = 256

goal_agent = GCSLAgent2(
    obs_dim, env.action_spec().shape, goal_dim, "cuda", lr, hidden_dim, max_horizon + 1
)
...
states = np.concatenate([e[0] for e in episodes])
physics = np.concatenate([e[2] for e in episodes])
state_physics = (states, physics)

from scipy.cluster.vq import kmeans

clusters, resid = kmeans(states[:250000], 100)
...
for tstates in (states[i : i + stride] for i in range(0, total, int(stride))):
