...
#            self.proto_goals = a[tmp_.clone().detach().cpu().numpy(), :2]
    ...
    def eval_intrinsic(self, encoded, states):
        ...
        df = pd.DataFrame()
        df['x'] = states[:,0].round(4)
        df['y'] = states[:,1].round(4)
        ...
        sns.heatmap(result, cmap="Blues_r",fmt='.4f', ax=ax).invert_yaxis()
        ax.set_xticklabels(['{:.4f}'.format(float(t.get_text())) for t in ax.get_xticklabels()])
        ax.set_yticklabels(['{:.4f}'.format(float(t.get_text())) for t in ax.get_yticklabels()])
