...
#         return metrics
    ...
    def update_actor(self, obs, action, step):
        ...
        action1 = self.expert_1.act(obs, stddev)
        action2 = self.expert_2.act(obs, stddev)
        action3 = self.expert_3.act(obs, stddev)
        action4 = self.expert_4.act(obs, stddev)
        actions = torch.vstack((action1, action2, action3, action4))
...
#             self.update_critic(obs, action, reward, discount, next_obs, step))
        ...
        for ix, x in enumerate([expert_1, expert_2, expert_3, expert_4]):
            if ix ==0:
                ...
                metrics.update(self.update_actor(obs, action, goal, x, step))
            elif ix ==1:
                goal = np.tile(np.array([-.15, .15]), 1024, 1)
                metrics.update(self.update_actor(obs, action, goal, x, step))
            elif ix ==2:
